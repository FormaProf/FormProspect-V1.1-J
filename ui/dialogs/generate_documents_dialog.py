...
class GenerateDocumentsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        subtitle = QLabel("Les documents sont fusionnés à partir de vos modèles Word officiels, sans reconstruction de la mise en page.")
    ...
    def _refresh_summary(self):
        ...
        preflight = self.service.preflight(int(s["session_id"]), selected)
        templates = "\n".join(f"• {kind} : {name}" for kind, name in preflight.templates.items()) or "• Aucun document sélectionné"
        self.summary.setPlainText(
            ...
            f"Prix : {s['price_cents']/100:,.2f} €\n\nMODÈLES UTILISÉS\n{templates}".replace(",", " ")
        )
        if preflight.errors:
            self.preflight_label.setText("⛔ À corriger avant génération :\n• " + "\n• ".join(preflight.errors))
            self.preflight_label.setStyleSheet("color:#B42318; background:#FEF3F2; border:1px solid #FDA29B; border-radius:8px; padding:9px;")
        elif preflight.warnings:
            self.preflight_label.setText("⚠ Vérifications :\n• " + "\n• ".join(preflight.warnings))
            self.preflight_label.setStyleSheet("color:#92400E; background:#FFFBEB; border:1px solid #FDE68A; border-radius:8px; padding:9px;")
        else:
            self.preflight_label.setText("✓ Dossier complet : la génération peut démarrer.")
            self.preflight_label.setStyleSheet("color:#067647; background:#ECFDF3; border:1px solid #ABEFC6; border-radius:8px; padding:9px;")
    ...
    def _generate(self):
        ...
        if preflight.errors:
            QMessageBox.warning(self, "Dossier incomplet", "Corrigez les éléments suivants :\n\n• " + "\n• ".join(preflight.errors))
            return
        if preflight.warnings:
            answer = QMessageBox.question(
                self, "Vérifications avant génération",
                "Les documents peuvent être générés, mais certains éléments sont à vérifier :\n\n• "
                + "\n• ".join(preflight.warnings) + "\n\nContinuer ?",
        ...
        lines = []
        for item in results:
            status = "DOCX + PDF" if item.pdf_path else "DOCX"
            line = f"• {item.document_type} {item.document_number} — {status} — {item.template_name}"
            if item.warning:
                line += f"\n  ⚠ {item.warning}"
            lines.append(line)

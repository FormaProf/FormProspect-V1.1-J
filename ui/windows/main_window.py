...
class MainWindow(QMainWindow):
    def __init__(self, auth_service: AuthService):
        ...
        print("[MAIN] Toutes les pages créées", flush=True)

        for page in (
            self.dashboard_page, self.treatment_page, self.prospects_page,
            self.agenda_page, self.campaigns_page, self.sequences_page, self.commissions_page, self.documents_page, self.trainers_page, self.trainer_availability_page, self.commercial_profiles_page, self.training_cases_page, self.admin_statistics_page, self.ai_assistant_page, self.activity_page, self.admin_account_page, self.account_page,
        ):
    ...
    def creer_menus(self):
        ...
        if SessionState.has_role("Administrateur"):
            ...
            action_trainer_availability.triggered.connect(
                self.ouvrir_disponibilites_formateurs
            )
